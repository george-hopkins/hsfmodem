...
static u32 azx_get_linkpos(snd_pcm_substream_t *substream)
{
...
//printk(KERN_DEBUG"%s: %p tag=%d bufsize=%d fmt=0x%x\n", __FUNCTION__, azx_dev, (int)azx_dev->stream_tag, azx_dev->bufsize, (unsigned int)azx_dev->format_val);
...
//	runtime->hw.fifo_size = azx_sd_readw(azx_dev, SD_FIFOSIZE);
...
//printk(KERN_ERR"%s: pci=%p card=%p\n", __FUNCTION__, pci, card);
	*rchip = NULL;
...
//printk(KERN_ERR"%s: pci=%p card=%p\n", __FUNCTION__, pci, pci_get_drvdata(pci));
