...
static int stac92xx_boolean_switch_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
#define stac92xx_aloopback_info stac92xx_boolean_switch_info
...
#define stac92xx_io_switch_info		stac92xx_boolean_switch_info
...
#define stac92xx_clfe_switch_info stac92xx_boolean_switch_info
...
static int stac92xx_add_control(struct sigmatel_spec *spec, int type, const char *name, unsigned long val)
{
	...
	*knew = stac92xx_control_templates[type];
	knew->name = kmalloc(namelen, GFP_KERNEL);
	if (! knew->name)
		return -ENOMEM;
	memcpy(knew->name, name, namelen);
