...
EXPORT_SYMBOL(snd_hda_codec_wallclock);
...
//printk("%s: %p calling codec->patch_ops.exit for codec %p\n", __FUNCTION__, bus, codec);
			codec->patch_ops.exit(codec);
		}
	}
}

EXPORT_SYMBOL(snd_hda_codec_remove_notify_all);
...
#if 0
static int snd_hda_bus_dev_disconnect(snd_device_t *device)
{
	printk("%s: %p\n", __FUNCTION__, device);
	return 0;
}

static int snd_hda_bus_dev_unregister(snd_device_t *device)
{
	printk("%s: %p\n", __FUNCTION__, device);
	return 0;
}
...
//printk("%s: %u %p afg=0x%04x mfg=0x%04x subsystem_id=%x preset=%p\n", __FUNCTION__, codec_addr, codec, codec->afg, codec->mfg, codec->subsystem_id, codec->preset);
				...
				unsigned int power_state)
{
	...
	snd_hda_codec_write(codec, fg, 0, AC_VERB_SET_POWER_STATE,
			    power_state);
	msleep(10); // partial workaround for "hda_intel: azx_get_response timeout, switching to polling mode..." issue
...
int snd_hda_suspend(struct hda_bus *bus, pm_message_t state)
{
	...
	//printk("%s: bus=%p state=0x%x\n", __FUNCTION__, bus,  *((u32 *)&state));
